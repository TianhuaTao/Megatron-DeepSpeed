...
def set_attn_state(args, layer, hf_layer):
    ...
    # q_vis = torch.arange(4096*4096).reshape(hf_attn.q_proj.weight.shape).float()
    # k_vis = torch.arange(4096*4096).reshape(hf_attn.q_proj.weight.shape) + q_vis.max()
    # v_vis = torch.arange(4096*4096).reshape(hf_attn.q_proj.weight.shape) + k_vis.max()
    # plt.imsave("q.png", q_vis.numpy())
    # plt.imsave("k.png", k_vis.numpy())
    # plt.imsave("v.png", v_vis.numpy())

    # plt.imsave("qkv_concat.png", torch.cat([q_vis, k_vis, v_vis], dim=0).numpy())


    # qkv_vis = torch.cat([ 
    #     q_vis.reshape((ng, dim*nh//ng, -1)),
    #     k_vis.reshape((ng, dim, -1)),
    #     v_vis.reshape((ng, dim, -1)),
    # ], dim=1).reshape((-1, args.hidden_size))
    # plt.imsave("qkv_vis.png", qkv_vis.numpy())
...
def _load_checkpoint(queue, args):
    ...
    check_for_arg('use_rotary_position_embeddings', True)
